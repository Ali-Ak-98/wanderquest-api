    ...
    getAllCities(filters?: any) {
        ...
        if (filters?.continent) {
            filteredCities = filteredCities.filter(
                city => city.continent.toLowerCase() === filters.continent.toLowerCase(),
            );
        }
        ...
        if (filters?.minPopulation) {
            filteredCities = filteredCities.filter(
                city => parseInt(city.population, 10) >= parseInt(filters.minPopulation, 10),
            );
        }
        ...
        if (filters?.maxPopulation) {
            filteredCities = filteredCities.filter(
                city => parseInt(city.population, 10) <= parseInt(filters.maxPopulation, 10),
            );
        }
        ...
        if (filters?.landmark) {
            filteredCities = filteredCities.filter(
                city => city.landmarks.some(l => l.toLowerCase().includes(filters.landmark.toLowerCase())),
            );
        }
